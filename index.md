---
layout: default
name: Derin Gezgin

# Profile photo
profile_photo: images/photo.jpg

# Introduction content
intro_content: |
  <p>
    I am currently a rising-junior at Connecticut College, double majoring in Computer Science & Statistics and Data Science.
  </p>
  <p>
    While pursuing my degrees in Computer Science and Data Science, I actively collaborate with my professors at Connecticut College on multiple research projects.
    At the same time, I work as a teaching assistant and grader for multiple Computer Science courses. I am also a member of the Computer Science Student Advisory Board as the Diversity Chair.
  </p>

# Links
links:
  - title: Email
    url: mailto:dgezgin@conncoll.edu
  - title: CV
    url: assets/Gezgin_Derin_CV.pdf
  - title: Google Scholar
    url: https://scholar.google.com/citations?user=kVhLnW8AAAAJ&hl=en
  - title: GitHub
    url: https://github.com/deringezgin
  - title: LinkedIn
    url: https://www.linkedin.com/in/deringezgin/

# Research Introduction
research_intro: |
  <p>
    I'm a member of two research labs at Connecticut College: <a href="https://oak.conncoll.edu/parker/research.html" target="_blank">Autonomous Agent Learning Lab</a> and <a href="https://www.informatics.digital.conncoll.edu" target="_blank">Informatics Lab</a>. My research focuses on computer vision, machine learning, and artificial intelligence.
  </p>

# Research Projects
projects:
  - title: A framework for river connectivity classification using temporal image processing and attention based neural networks
    image: images/river_connectivity.png
    authors: Timothy James Becker, <strong>Derin Gezgin</strong>, Jun Yi He Wu, Mary Becker
    publication: ACM Conference on Computing and Sustainable Societies, 2025
    links:
      - title: arXiv
        url: https://arxiv.org/abs/2502.00474
        color: "#b31b1b"
      - title: Code
        url: https://github.com/deringezgin/River-Connectivity-Classification
        color: "#24292e"
  - title: Evolutionary Optimization of Deep Learning Agents for Sparrow Mahjong
    image: images/sparrow.jpg
    authors: Jim O'Connor, <strong>Derin Gezgin</strong>, Gary B. Parker
    publication: In Review
  - title: Evolving Neural Controllers for Xpilot-AI Racing Using Neuroevolution of Augmenting Topologies
    image: images/xprace.png
    authors: Jim O'Connor, Nicolas Lorentzen, Gary B. Parker, <strong>Derin Gezgin</strong>
    publication: In Review
--- 